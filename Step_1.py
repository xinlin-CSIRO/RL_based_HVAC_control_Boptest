...
result_location = "C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\RL_WORK_SUMMARY\\step_2.csv"
    ...
    def learn(self, total_episodes):
        ...
        for i in range(total_episodes):
            ...
            while not done:
                ...
                # print('reward is: ', rew)
                supply_sp= np.array(env.val_bins_act)[0,action[0]]
                return_sp=nxt_obs[0]
                heat_power=nxt_obs[1]
                cool_power=nxt_obs[2]
                out_weather=nxt_obs[3]
                DNI = nxt_obs[4]
                record_s = str(supply_sp)+','+ str(return_sp)+','+str(heat_power)+','+str(cool_power)+','+ str(out_weather)+','+ str(DNI)+'\n'#'set,return,heat, cool, outside/n'
...
env = BoptestGymEnv(            url                   = url,
                                ...
                                observations          = {'zon_reaTRooAir_y':(250,303),
