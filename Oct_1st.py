...
class BoptestGymEnvCustomReward(BoptestGymEnv):
    def compute_reward(self, action):
        ...
        predicted_outdoor=0
        if(len(outdoor_observation)==201):
            outdoor_air_training=outdoor_observation[0:200]
            wind_speed_training=wind_speed_observation[0:200]
            train_X=np.array([outdoor_air_training,wind_speed_training])
            train_Y=np.array(outdoor_observation[1:201])
        ...
        if (indoor_states == 0):
            ...
            thermal_r=10*penality_(diff_real)
            scenario = 3
        ...
        result_sting = str(indoor_air_temp) +','+str(low_boundary)+ ','+str(up_bundary) + ','+ str(action_)+','+str(reward)+','+ str(out_door_air) + ',' + str(thermal_r) +\
                       ',' +  str(energy_r) +','+str(thermal_kpi)+','+str(energy_usage_kpi)+','+str(indoor_states)+','+str(predicted_outdoor)+','+str(last_prediction_error)+','+str(final_prediction)+'\n'
