...
_n_days_ = 300
test_scenario = 'peak_heat_day'
now = datetime.now()
current_time = now.strftime("%dth-%b-%H-%M")
# current_time2= now.strftime("%H:%M")
result_learning = "C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\RL_WORK_SUMMARY\\best_heat_baseline_" + current_time + "_ISGT.csv"
...
record = 'indoor_temp,boundary_0,boundary_1,outdoor_air,energy KPI, thermal dis KPI\n'
f_1.write(record)
f_1.close()
...
env = BoptestGymEnvCustomReward(url=url,
                    ...
                    predictive_period  =0,
                    step_period=30 * 60,
                    start_time = 13 * 24 * 3600,
                    max_episode_length=300000 * 24 * 3600,
...
for x in range(0, test_steps):
    ...
    result_sting = str(indoor_air_temp) + ',' + str(low_boundary) + ',' + str(up_bundary) + ',' + str(out_door_air)  + ','+str(energy_usage_kpi) + ','+str(thermal_kpi)+ '\n'
    result_sting.replace('[', '').replace(']', '')
    f_1 = open(result_learning, "a+")
    f_1.write(result_sting)
    f_1.close()
