...
from wrapper_xinlin import DiscretizedActionWrapper_xinlin_,ContinuousActionWrapper_xinlin,ContinuousActionWrapper_xinlin_single_action,ContinuousActionWrapper_xinlin_single_action_setpoint, ContinuousActionWrapper_xinlin_two_rooms
...
_n_days_=300
test_scenario = 'typical_heat_day' #'peak_heat_day'
now = datetime.now()
current_time= now.strftime("%dth-%b-%H-%M")
result_learning = "C:\\Users\\wan397\\OneDrive - CSIRO\\Desktop\\RL_WORK_SUMMARY\\Best_Two_rooms_our_method_"+current_time+"_SAC_Feb.csv"
...
record='living room temp,boundary,boundary,bedroom temp, action_0, action_1,reward,livingroom reward, bedroom reward,themal kpi,energy kpi,outdoor air,scenario,target,energy usage,thermal_weight\n'
f_1.write(record)
f_1.close()
...
class BoptestGymEnvCustomReward(BoptestGymEnv):
    ...
    def compute_reward(self, action):
        ...
        living_room = observations[0] - 273.15
        bed_room = observations[1] - 273.15
        out_door_air = observations[2] - 273.15
        heat_consumption = observations[3]
        low_boundary_living = observations[4] - 273.15
        up_bundary_living = observations[5] - 273.15

        # living_room = observations[0] - 273.15
        # out_door_air = observations[1] - 273.15
        # heat_consumption = observations[2]
        # low_boundary_living = observations[3] - 273.15
        # up_bundary_living = observations[4] - 273.15
        ...
        R_living, r_t, r_e, current_indoor_state, thermal_weight, target, scenario = the_one_under_test_outdoor  (
           low_boundary_living,
           up_bundary_living,
           living_room,
           heat_consumption,
        ...
        R_bed, r_t, r_e, current_indoor_state, thermal_weight, target, scenario = the_one_under_test_outdoor(
            low_boundary_living,
            up_bundary_living,
            bed_room,
            heat_consumption,
        ...
        result_sting = str(living_room) + ',' + str(low_boundary_living) + ',' + str(up_bundary_living) + ',' + \
                       str(bed_room) + ','  + str(
            action_0) + ',' + str(action_1)+','+str(final_reward) + ',' + str(R_living) + ',' + str(R_bed) + ',' + str(
...
env = BoptestGymEnvCustomReward(url                   = url,
                                testcase              = 'twozone_apartment_hydronic', #'bestest_hydronic_heat_pump',
                                actions               = {
                                                            'thermostatDayZon_oveTsetZon_u':(288.15,303.15),
                                                            'thermostatNigZon_oveTsetZon_u':(288.15,303.15),
                                                         },
                                observations          = {
                                                        'dayZon_reaTRooAir_y': (250, 303),# zone air temp
                                                        'nigZon_reaTRooAir_y': (250, 303),# zone air temp
                                                        'weatherStation_reaWeaTDryBul_y': (250, 303),  # outside air temp
                                                        'hydronicSystem_reaPeleHeaPum_y': (0, energy_consumption_up_bond),# heating energy consumption
                                                        'LowerSetp[Day]':(280, 310),
                                                        'UpperSetp[Day]':(280, 310),
                                                        # 'LowerSetp[Night]':(280, 310),
                                                        # 'UpperSetp[Night]':(280, 310),
                                                         },
                                ...
                                excluding_periods = [(start_date*24*3600,  start_date*24*3600+days_in_feb_2017*24*3600)]
                                )
...
env = ContinuousActionWrapper_xinlin_two_rooms (env)
# env = ContinuousActionWrapper_xinlin_single_action_setpoint (env)
...
# model = A2C('MlpPolicy', env, learning_rate=0.01, n_steps=5 )

model = SAC("MlpPolicy", env, learning_rate=0.001, batch_size =48 )

learning_steps = 1*4*24*100 # (100 days)
test_steps = 1*4*24*(days_in_feb_2017)
